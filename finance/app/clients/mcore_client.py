...
class McoreClient:
    ...
    def check_and_report(self, task: dict) -> bool:
        ...
        if task.get("taskData") is None:
            error_msg =  "taskData is empty"
        elif task["taskData"].get("fin") is None:
            error_msg =  "taskData.fin is empty"
        elif task["taskData"]["fin"].get("command") is None:
            error_msg =  "taskData.fin.command is empty"
        elif task["taskData"]["fin"]["command"] != "run": ## TODO need more then once command
            error_msg =  "transmission taskData.fin.command is not run"
        ...
        return self.report_task(task["id"], 3, "finance get the job: " + str(task["id"]))
